...
const TimeSlots = ({ expertId }: { expertId: string }) => {
  ...
  useEffect(() => {
    if (timeSlotsData?.timeSlots) {
      ...
      console.log("availableTimeSlots", availableTimeSlots.length, timeSlotsData.timeSlots.length)
      ...
      if (minTimeDuration && availableTimeSlots.length > 0) {
        ...
        console.log("time slots after setting ", timeSlots)
      }
  ...
  //     while (isBefore(addMinutes(currentTime, duration), endTime) || getTime(addMinutes(currentTime, duration)) === getTime(endTime)) {
  //       generatedTimeSlots.push({
  //         availableTimings: [
  //           {
  //             startingTime: new Date(currentTime),
  //             endingTime: addMinutes(currentTime, duration),
  //           },
  //         ],
  //       });
  //       currentTime = addMinutes(currentTime, duration);
  ...
  console.log("time slots in doctor profile", timeSlots.length)
  console.log("time slots data", timeSlotsData)
  console.log("time duration", timeDurations, minTimeDuration )
  ...
  return (
    <>
      ...
      <div className="mt-[10px]  flex items-center">
        ...
        {days.map((day) => (
          <span
            ...
            key={day.toISOString()}
            className={`mx-[10px] pb-[6px]  font-inter text-xs font-normal sm:text-sm 2xl:text-base ${
      ...
      <div className="mt-[18px] max-h-[85px] overflow-y-auto ">
        {timeSlots.length > 0 ? (
